...
// 主进程菜单「报告问题」→ menu-report-issue → 打开本弹窗
const onMenuReportIssue = () => open()

onMounted(() => {
  window.ipcRenderer?.on('menu-report-issue', onMenuReportIssue)
})

onUnmounted(() => {
  window.ipcRenderer?.off('menu-report-issue', onMenuReportIssue)
})
