...
const handleSave = (row, index) => {
  ...
  if (props.rules) {
    for (const col of props.columns) {
      if (props.rules[col.prop]) {
        const rules = props.rules[col.prop]
        ...
        for (const rule of rules) {
          if (rule.required) {
            if (
              ...
              ElMessage.warning(rule.message || `请填写${col.label}`)
              return
            }
          }
          if (rule.validator) {
            let isValid = true
            // 模拟 element-plus validator 的调用方式: (rule, value, callback)
            // 注意：这里我们是同步校验，callback 需要立即执行或我们手动处理错误
            try {
          ...
          if (rule.pattern) {
            if (!rule.pattern.test(value)) {
              ElMessage.warning(rule.message || `${col.label}格式不正确`)
              return
            }
          }
        }
