...
;(function () {
  ...
  function checkHtmlPatterns(html) {
    ...
    const pureBlackBgRe =
      /background(?:-color)?\s*:\s*(?:#000000|#000|rgb\(\s*0,\s*0,\s*0\s*\))\b/gi
  ...
  function checkPageLayout(doc, win) {
    ...
    // Everything centered
    const textEls = doc.querySelectorAll(
      'h1, h2, h3, h4, h5, h6, p, li, div, button'
    )
    let centeredCount = 0
    let totalText = 0
    for (const el of textEls) {
      ...
      while (cur && cur.nodeType === 1) {
        ...
        if (
          /text-align\s*:\s*center/i.test(rawStyle) ||
      ...
      for (const { el, findings } of allFindings) {
        for (const f of findings) {
          console.log(
            ...
            'color: oklch(55% 0.25 350); font-weight: bold',
            'color: inherit',
            el
          )
        }
    ...
    function collectBrowserFindings() {
      ...
      for (const el of document.querySelectorAll('*')) {
        ...
        const findings = [
          ...
          ...checkElementHeroEyebrowDOM(el).map((f) => ({
            type: f.id,
            detail: f.snippet
          })),
